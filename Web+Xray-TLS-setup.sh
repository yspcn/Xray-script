...
nginx_is_installed=""
redis="$(curl -H 'Cache-Control: no-cache' -s "https://api.github.com/repos/phpredis/phpredis/releases/latest" | grep 'tag_name' | cut -d\" -f4)"
    if [[ ! -n "$redis" ]]; then
        redis=5.3.7
    fi
apcu="$(curl -H 'Cache-Control: no-cache' -s "https://api.github.com/repos/krakjoe/apcu/releases/latest" | grep tag_name | sed -E 's/.*"v(.*)".*/\1/')"
    if [[ ! -n "$apcu" ]]; then
        apcu=5.1.22
    fi
...
cloudreve_version="$(curl -H 'Cache-Control: no-cache' -s "https://api.github.com/repos/cloudreve/Cloudreve/releases/latest" | grep 'tag_name' | cut -d\" -f4)"
cloudreve_prefix="/usr/local/cloudreve"
cloudreve_service="/etc/systemd/system/cloudreve.service"
unset cloudreve_is_installed

nextcloud_url="https://download.nextcloud.com/server/releases/nextcloud-24.0.10.tar.bz2"
...
check_script_update()
{
    [ "$(md5sum "${BASH_SOURCE[0]}" | awk '{print $1}')" == "$(md5sum <(wget -O - "https://github.com/yspcn/Xray-script/raw/main/Web+Xray-TLS-setup.sh") | awk '{print $1}')" ] && return 1 || return 0
}
#更新脚本
update_script()
{
    if wget -O "${BASH_SOURCE[0]}" "https://github.com/yspcn/Xray-script/raw/main/Web+Xray-TLS-setup.sh" || wget -O "${BASH_SOURCE[0]}" "https://github.com/yspcn/Xray-script/raw/main/Web+Xray-TLS-setup.sh"; then
    ...
    if [ $release == "ubuntu" ] || [ $release == "debian" ] || [ $release == "deepin" ] || [ $release == "other-debian" ]; then
        if LANG="en_US.UTF-8" LANGUAGE="en_US:en" dpkg -s "$1" 2>/dev/null | grep -qi 'status[ '$'\t]*:[ '$'\t]*install[ '$'\t]*ok[ '$'\t]*installed[ '$'\t]*$'; then
            if LANG="en_US.UTF-8" LANGUAGE="en_US:en" apt-mark manual "$1" | grep -qi 'set[ '$'\t]*to[ '$'\t]*manually[ '$'\t]*installed'; then
                ...
                red "安装依赖 \"$1\" 出错！"
                green  "欢迎进行Bug report(https://github.com/yspcn/Xray-script/issues)，感谢您的支持"
...
swap_off()
{
    if [ $using_swap_now -eq 1 ]; then
        ...
        if swapoff ${temp_dir}/swap && rm -rf ${temp_dir}/swap; then
            ...
            red    "关闭swap失败！"
            green  "欢迎进行Bug report(https://github.com/yspcn/Xray-script/issues)，感谢您的
支持"
            yellow "按回车键继续或者Ctrl+c退出"
            read -s
        fi
    ...
    updateSystem()
    {
        ...
        if [ $choice -ne 0 ]; then
            ...
            if [ $in_install_update_xray_tls_web -eq 1 ]; then
                echo
                tyblue "提示：即将开始升级系统"
                yellow " 升级完系统后服务器将重启，重启后，请再次运行脚本完成 Web+TLS+Web 剩余部分的安装/升级"
                yellow " 再次运行脚本时，重复之前选过的选项即可"
                echo
                sleep 2s
                yellow "按回车键以继续。。。"
    ...
    do
        ...
        if (( 1<=choice&&choice<=4 )); then
            if (( choice==1 || choice==4 )) && ([ $release == "ubuntu" ] || [ $release == "debian" ] || [ $release == "deepin" ] || [ $release == "other-debian" ]) && ! dpkg-deb --help | grep -qw "zstd"; then
                ...
                if (( choice==1 || choice==4 )) && ([ $release == "ubuntu" ] || [ $release == "debian" ] || [ $release == "deepin" ] || [ $release == "other-debian" ]) && ! version_ge "$(dpkg --list | grep '^[ '$'\t]*ii[ '$'\t][ '$'\t]*linux-base[ '$'\t]' | awk '{print $3}')" "4.5ubuntu1~16.04.1"; then
                    ...
                    if [ $in_install_update_xray_tls_web -eq 1 ]; then
                        echo
                        tyblue "提示："
                        yellow " 更换内核后服务器将重启，重启后，请再次运行脚本完成 Web+TLS+Web 剩余部分的安装/升级"
                        yellow " 再次运行脚本时，重复之前选过的选项即可"
                        echo
                        sleep 2s
                        yellow "按回车键以继续。。。"
            ...
            if [ $in_install_update_xray_tls_web -eq 1 ]; then
                ...
                yellow "    3. 选择 \"退出bbr安装\" 选项完成 Web+TLS+Web 剩余部分的安装/升级"
            else
...
readProtocolConfig()
{
    ...
    blue   "   2. gRPC和WebSocket支持通过CDN，关于两者的区别，详见：https://github.com/yspcn/Xray-script#关于grpc与websocket"
    blue   "   3. 仅TCP能使用XTLS"
    echo
    local choice=""
...
readPretend()
{
    ...
    do
        ...
        red    " 5. 自定义反向代理网页 (不推荐)"
		green  " 6. 安装wordpress（推荐先安装nextcloud再安装）"
        echo
        green  " 内存<128MB 建议选择 403页面"
        green  " 128MB<=内存<1G 建议选择 Cloudreve"
        green  " 内存>=1G 建议选择 Nextcloud 或 Cloudreve"
        echo
        yellow " 关于选择伪装网站的详细说明见：https://github.com/yspcn/Xray-script#伪装网站说明"
        echo
        pretend=""
        while [[ "$pretend" != "1" && "$pretend" != "2" && "$pretend" != "3" && "$pretend" != "4" && "$pretend" != "5" && "$pretend" != "6" ]]
        do
...
instal_php_apcu()
{
    if ! wget http://pecl.php.net/get/apcu-${apcu}.tgz; then
        yellow "获取php-apcu源码失败"
        yellow "按回车键继续或者按Ctrl+c终止"
        read -s
    fi
    tar -zvxf apcu-${apcu}.tgz
    cd apcu-${apcu}
    ${php_prefix}/bin/phpize
...
init_web()
{
    ...
    if [ "${pretend_list[$1]}" == "1" ]; then
        ...
        turn_on_off_php
        turn_on_off_cloudreve
	elif [ "${pretend_list[$1]}" == "6" ]; then
       if ! wget -O "${nginx_prefix}/html/latest.zip" https://wordpress.org/latest.zip ; then
            red    "获取wordpess失败"
            yellow "按回车键继续或者按Ctrl+c终止"
            read -s
        fi
        rm -rf "${nginx_prefix}/html/${true_domain_list[$1]}"
        unzip -q -d "${nginx_prefix}/html" "${nginx_prefix}/html/latest.zip"
        rm -f "${nginx_prefix}/html/latest.zip"
...
print_share_link()
{
    ...
    if [ $protocol_1 -eq 1 ] || [ $protocol_1 -eq 3 ]; then
        ...
        do
            if [ "${pretend_list[$i]}" == "1" ] || [ "${pretend_list[$i]}" == "2" ]; then
                tyblue "vless://${xid_1}@${ip}:10086?security=tls&sni=${domain_list[$i]}&alpn=http%2F1.1&flow=xtls-rprx-vision"
            else
                tyblue "vless://${xid_1}@${ip}:10086?security=tls&sni=${domain_list[$i]}&alpn=h2,http%2F1.1&flow=xtls-rprx-vision"
            fi
        done
    fi
    if [ $protocol_1 -eq 2 ] || [ $protocol_1 -eq 3 ]; then
        ...
        do
            if [ "${pretend_list[$i]}" == "1" ] || [ "${pretend_list[$i]}" == "2" ]; then
                tyblue "vless://${xid_1}@${ip}:100863?security=tls&sni=${domain_list[$i]}&alpn=http%2F1.1"
            else
                tyblue "vless://${xid_1}@${ip}:10086?security=tls&sni=${domain_list[$i]}&alpn=h2,http%2F1.1"
            fi
        done
    fi
    if [ $protocol_2 -eq 1 ]; then
...
print_config_info()
{
    ...
    echo
    yellow " 关于fingerprint与alpn，详见：https://github.com/yspcn/Xray-script#关于tls握手tls指纹和alpn"
    echo
...
install_update_xray_tls_web()
{
    ...
    local install_php
    if [ $update -eq 0 ]; then
        [ "${pretend_list[0]}" == "2" ] || [ "${pretend_list[0]}" == "4" ] || [ "${pretend_list[0]}" == "6" ] && install_php=1 || install_php=0
    else
    ...
    if [ $update -eq 0 ]; then
        green "即将开始安装Web+TLS+Web，可能需要10-20分钟。。。"
        sleep 3s
    fi
...
reinit_domain()
{
    ...
    get_config_info
    readDomain
    if [ "${pretend_list[-1]}" == "2" ] || [ "${pretend_list[-1]}" == "4" ] || [ "${pretend_list[-1]}" == "6" ] && [ $php_is_installed -eq 0 ]; then
        check_SELinux
        check_important_dependence_installed "procps" "procps-ng"
        install_epel
...
change_pretend()
{
    ...
    if [ "$pretend" == "2" ] || [ "$pretend" == "4" ] || [ "$pretend" == "6" ] && [ $php_is_installed -eq 0 ]; then
        check_SELinux
        check_important_dependence_installed "procps" "procps-ng"
        install_epel
...
simplify_system()
{
    ...
    tyblue " 2. 如果VPS上部署了 Web+TLS+Web 以外的东西，可能被误删"
    ! ask_if "是否要继续?(y/n)" && return 0
    echo
    local save_ssh=0
...
start_menu()
{
    ...
    echo
    tyblue "       官网：https://github.com/yspcn/Xray-script"
    echo
    tyblue "----------------------------------注意事项----------------------------------"
    yellow " 1. 此脚本需要一个解析到本服务器的域名"
    tyblue " 2. 此脚本安装时间较长，建议在安装前阅读："
    tyblue "      https://github.com/yspcn/Xray-script#安装时长说明"
    ...
    tyblue "   9. 更新Xray"
    red    "  10. 卸载Web+TLS+Web"
    red    "  11. 卸载php"
    red    "  12. 卸载Cloudreve"
    echo
    tyblue " --------------启动/停止-------------"
    tyblue "  13. 启动/重启Web+TLS+Web"
    tyblue "  14. 停止Web+TLS+Web"
    echo
    tyblue " ----------------管理----------------"
    tyblue "  15. 查看配置信息"
    tyblue "  16. 重置域名"
    ...
    tyblue " ----------------其它----------------"
    tyblue "  25. 精简系统"
    purple "         删除不必要的系统组件，即使已经安装 Web+TLS+Web 仍然可以使用此功能"
