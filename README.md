...
删除自定义模板页改为wordpress安装，前提是必须要先安装一个nextcloud做主站，支持一键安装wordpress，安装语言自己选
## 目录
[1. 脚本特性](#脚本特性)
...
### 2. 获取/更新脚本
```bash
wget -O Xray-TLS+Web-setup.sh --no-check-certificate https://github.com/eysp/Xray-script/raw/main/Xray-TLS+Web-setup.sh
```
### 3. 执行脚本
```bash
